...
export class OAuth2Client {
   ...
   *
   * @param {object} options - Options to use.
   * @param {string} options.onInit - If the requester is calling on
   *  initialization.
   * @returns {Promise<string>} The access token.
   */
  async getAccessToken({onInit = false}) {
    const {
      client_id, client_secret, token_endpoint, grant_type, scope, httpsAgent,
      maxRetries
    } = this;
    if(onInit) {
      return getAccessTokenOnInit({
        client_id, client_secret, token_endpoint, grant_type, scope, httpsAgent
      });
    }
...
async function getAccessTokenOnInit({
  client_id, client_secret, token_endpoint, grant_type, scope, httpsAgent
}) {
  const body = new URLSearchParams({
    client_id, client_secret, grant_type, scope
  ...
  let accessToken;
  try {
    accessToken = await pRetry(() => _requestAccessTokenOnInit({
...
async function _requestAccessTokenOnInit({url, body, httpsAgent}) {
  let response;
  try {
    ({data: response} = await httpClient.post(url, {
      body,
